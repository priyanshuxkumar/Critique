...
export default function Page() {
      ...
      return async (e: Event) => {
        try {
          ...
          const response = await axios.post(
            "http://localhost:8000/api/v1/website/get-signed-url/website-icon",
            {
              imageName: file.name,
              imageType: file.type,
            },
  ...
  async function handleAddWebsite() {
    try {
      const response = await axios.post(
        `http://localhost:8000/api/v1/website/add`,
        {
          ...addWebsiteData,
        },
        {
    ...
    <>
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 py-7">
          <div className="flex justify-between gap-2 mb-8">
            <div>
              ...
              <p className="text-muted-foreground">
                Manage and review your websites in one place
              </p>
            </div>
          ...
          <div className="mb-6">
            <h2 className="text-xl font-medium mb-4">Websites</h2>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredWebsites.map((site) => (
                  <Card
                    onClick={() => router.push(`/website/${site.id}/reviews`)}
                    key={site.id}
                    className="overflow-hidden transition-all shadow-none hover:shadow-md cursor-pointer"
                  >
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground line-clamp-3">
                        This website is available for review. Click the button
                        below to see detailed analytics and performance metrics.
                      </p>
                    </CardContent>
